...
timeSDM<-function(species, ncores, memory, nocc, sr, testingFrac = 0.2, plot_prediction=F, pollen_threshold='auto',
  ...
  if (modelMethod == 'GBM-BRT') {
    ...
    x <- as.matrix(training_set[c('bio2', 'bio7', 'bio8', 'bio15', 'bio18', 'bio19')])
    y <- training_set[['presence']]
    model <- randomForest(x, y, ntree=rfTrees)
  }
...
sequential_timer <- list()
for (i in 1:10){
  print(paste("Sequential: ", i))
  sequential_timer[[i]] <- timeSDM("Picea", nCores, 4, 2500, 0.5, modelMethod="SRF", rfTrees=10000, plot_prediction = T)
}
parallel_timer <- list()
for (i in 1:10){
  print(paste("Parallel: ", i))
  parallel_timer[[i]] <- timeSDM("Picea", nCores, 4, 2500, 0.5, modelMethod="PRF", rfTrees=10000, plot_prediction = T)
}
save(sequential_timer, file="seqlRF.RData")
save(parallel_timer, file="parRF.RData")
s <- vector()
for (i in 1:10){
  s[i] <- sequential_timer[[i]][3]
}
p <- vector()
for (i in 1:3){
  p[i] <- parallel_timer[[i]][3]
}
