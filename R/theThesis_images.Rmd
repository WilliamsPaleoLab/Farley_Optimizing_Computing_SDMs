...
library(knitr)
library(matrixStats)
options(java.parameters = "-Xmx2500m") ## change memory allotment to RJava
library(bartMachine)
...
print(paste("Accuracy Model Percent Variance Explained: ", gam.r2, "%"))
...
mars.mdDelta.RSS <- sum((mars.mdDelta.RSS)^2)
...
for (i in 1:length(names(mars.training.predictors.acc))){
  predName = mars.training.predictors.acc[i]
  predSet <- mars.training.predictors.acc[-predName]
  
  model <- bartMachine(predSet, mars.training.response.acc)
  
  p <- predict(model, mars.testing.predictors[predNames])
  
  r2 <- cor(p, mars.testing.acc$testingAUC)^2
  mse <- sum((p - mars.testing.acc[['testingAUC']]) ^ 2) / length(mars.testing.acc[['testingAUC']])
...
rf.mse.acc <- rf.mdDelta.acc / length(rf.prediction.acc)
