...
ggplot(predMat, aes(x = obs)) +
  ...
  ggtitle("GBM-BRT Model-Data Comparison")
...
calcTimeCostIndex <- function(trainingExamples, cells, numPredictors,
                            model, prices, rfTrees = 100, gbm.learningRate=0.01, gbm.treeComplexity=5){
...
res <- read.csv("/Users/scottsfarley/documents/thesis-scripts/data/gbm_all.csv")
nTest <- 50
test <- res[sample(nrow(res), nTest),]

predictors <- test[c("trainingExamples", "cells", "numPredictors", "treeComplexity", "learningRate", "X")]
predictors$model <- "GBM-BRT"

timeCosts <- list()
optima <- list()
for (i in 1:nTest){
  print(paste("Experiment Summary: ID#", i))
  testCase <- predictors[i,]
  ...
  timeCosts[[i]] <- timeCost
  optimal <- estimateOptimal(timeCost, plot.scene=F)
...
optimaDF <- data.frame(matrix(unlist(optima), ncol=5, byrow=T))
names(optimaDF) <- c("Cores", "Memory", "Seconds", "Cost", "InverseAcc")

summary(optimaDF)

ggplot(optimaDF) + 
  geom_histogram(aes(x = Cores, col='Cores', alpha=0.25)) + 
  geom_histogram(aes(x = Memory, col='Memory', alpha=0.25))


ggplot(optimaDF) + 
  geom_density(aes(x = Cost, col='Cores', alpha=0.25))
