...
# This is the Optimization Demonstration for GAM
...
```{r subset}
## Get a whole bunch of possible combinations ready to model with
cores <- seq(1, 10)
GBMemory <- seq(1, 10)
...
```{r}
## predict time and accuracy for a whole bunch of possible scenarios
...
# ggtitle("GAM Time/Cost Surface for Accuracy-Maximizing Point") +
# xlab("Cost (cents)") +
# ylab('Cost (seconds)') 
```
...
ggplot(dend_data$segments) + 
  ...
  scale_color_discrete() +   ggtitle("GAM Unconstrained Optimal Clustering")
...
ggplot(candidateConfigs) + 
  ...
  ggtitle("GAM Data-Contrainted Optimal") +
  ylab("Seconds") +
  xlab("Cost")
...
p <- predict(GAM.rf, subsetScenario)
...
```{r}
time.post <- bart_machine_get_posterior(GAM.rf, subsetScenario)$y_hat_posterior_samples
time.post <- data.frame(time.post)
...
p <- data.frame(allDistances)
p$value <- as.numeric(allDistances$value)
p$L1 <- as.factor(p$L1)
ggplot(p) + 
  geom_density(aes(x=value, group=L1, col=L1), alpha=0.025) +
  scale_fill_brewer('Dark2', guide=FALSE) + guides(colour=FALSE, L1=F) +
...
ggplot(dend_data$segments) + 
  ...
  axis.text.x = element_blank(),
  axis.ticks = element_blank()) +
  scale_color_discrete() +   ggtitle("GAM Data-Constrained Clustering")
...
write.csv(accMaxSet, "/users/scottsfarley/documents/thesis-scripts/data/gam_constrained_accMax.csv")
```
...
time.post <- bart_machine_get_posterior(GAM.rf, accMaxSet[c("numPredictors", "trainingExamples", "cores", "GBMemory", "cells")])
```
...
ggplot(dend_data$segments) + 
  ...
  axis.text.x = element_blank(),
  axis.ticks = element_blank()) +
  scale_color_discrete() +   ggtitle("GAM Cost-Constrained Optimal Clustering")
