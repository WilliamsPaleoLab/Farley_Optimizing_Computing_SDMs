...
candidates <- unique(post.timeCost.sorted$L1)[1:150]
...
groups <- cutree(fit, h=1) # cut tree into 5 clusters
...
firstCluster <- candidateConfigs[candidateConfigs$cluster == 1, ]
...
scenarioOut <- scenarioOut[scenarioOut$seconds < threshold.time & 
                                   scenarioOut$cost < threshold.cost, ]

if(nrow(scenarioOut) == 0){
  warning("No candidate hardware found matching those thresholds.")
}else{
  print(paste(nrow(subsetScenario), "candidates fit within these thresholds."))
}
```
...
s <- scenarioOut[c("trainingExamples", "numPredictors", "cores", "GBMemory", "cells")]
time.post <- bart_machine_get_posterior(GAM.rf, s)$y_hat_posterior_samples
time.post <- data.frame(time.post)
...
post.timeCost.sorted <- p[order(p$value),]
candidates <- unique(post.timeCost.sorted$L1)[1:15]
